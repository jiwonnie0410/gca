			...
			.ready(
					function() {
						...
						$(document)
								.on(
										...
										function() {
											...
											var check = false;
											var reply_id = $(this).attr(
													"reply_id");
											var r_type = $(this).attr("r_type");
											var reply_password = "reply_password_"
													+ reply_id;

											if ($("#" + reply_password).val()
													.trim() == "") {
												alert("패스워드을 입력하세요.");
										...
										function() {
											...
											if (check) {
												...
												var txt_reply_content = $(this)
														.parent().prev().prev()
														.prev().html().trim(); //댓글내용 가져오기
												if (r_type == "sub") {
													txt_reply_content = txt_reply_content
															.replace("→ ", "");//대댓글의 뎁스표시(화살표) 없애기
												}

												var txt_reply_writer = $(this)
														.parent().prev().prev()
														.html().trim(); //댓글작성자 가져오기

												//입력받는 창 등록
												var replyEditor = '<tr id="reply_add" class="reply_modify">'
														+ '   <td width="820px">'
														+ '       <textarea name="reply_modify_content_'+reply_id+'" id="reply_modify_content_'+reply_id+'" rows="3" cols="50">'
														+ txt_reply_content
														+ '</textarea>'
														+ //기존 내용 넣기
														'   </td>'
						...
						$(document)
								.on(
										"click",
										"button[name='reply_modify_cancel']",
										function() {
											//원래 데이터를 가져온다.
											var r_type = $(this).attr("r_type");
											var r_content = $(this).attr(
													"r_content");
											var r_writer = $(this).attr(
													"r_writer");
											var reply_id = $(this).attr(
													"reply_id");
											var parent_id = $(this).attr(
...
<body>
	<input type="hidden" id="board_id" name="board_id"
		value="${boardView.id}" />
	<div align="center">
		<h2 align="center">홍보게시판 상세 읽기</h2>
		<input type="hidden" id="board_id" name="board_id"
			value="${boardView.id}" />
		<div class=”table-responsive“>
			<table class="table">
				<tr>
					<td>제목: <input type="text" id="subject" name="subject"style="width: 280px;" placeholder="제목"value="${boardView.subject}" /> 
					작성자: <input type="text" id="writer" name="writer" style="width: 100px;" maxlength="10" placeholder="작성자" value="${boardView.writer}" /> 
						작성일: <input type="date" id="writer" name="writer" style="width: 100px;" maxlength="10" placeholder="작성자" value="${boardView.writer}" />
					</td>
				</tr>
				<tr>
					<td>내용: <textarea name="content" id="content" rows="10"
							cols="80">${boardView.content}</textarea>
					</td>
				</tr>
			</table>
			<!-- 댓글  보기-->
				...
				<c:forEach var="replyList" items="${replyList}" varStatus="status">
					<tr reply_type="all">
						...
						<td align="center"><c:if test="${replyList.depth != '1'}">
							...
							<button class="btn btn-primary px-5 py-3 mt-3"
								name="reply_modify" parent_id="${replyList.parent_id}"
								r_type="<c:if test="${replyList.depth == '0'}">main</c:if><c:if test="${replyList.depth == '1'}">sub</c:if>"
								reply_id="${replyList.reply_id}">수정</button>
							<button class="btn btn-primary px-5 py-3 mt-3" name="reply_del"
								r_type="<c:if test="${replyList.depth == '0'}">main</c:if><c:if test="${replyList.depth == '1'}">sub</c:if>"
			...
			<table class="table">
				<tr>
					<td width="400"><textarea id="reply_content"
							name="reply_content" rows="3" cols="60" placeholder="댓글을 입력하세요."></textarea></td>
					<td width="100">
						<button class="btn btn-primary px-3 py-1 mt-1" id="reply_save"
							name="reply_save">댓글 등록</button>
					</td>
				</tr>
			</table>
			<div align="center">
				<button class="btn btn-primary px-4 py-2 mt-3" id="modify"
					name="modify">글 수정</button>
				<button class="btn btn-primary px-4 py-2 mt-3" id="delete"
					name="delete">글 삭제</button>
